...
d3.csv("https://raw.githubusercontent.com/amyl1/ProgAssignment1920/master/data.csv", function(data) {
  ...
  orbRadii.forEach(function(item){
    ...
    setInterval(function () {
        ...
        var interpolateDay = d3.interpolate(day.endAngle()(), (2 * Math.PI * Math.random(), 2 * Math.PI * Math.random())); 
        ...
        d3.transition().tween("orbit", function () {
          return function (t) {
            ...
            // Transition Earth
            d3.select(".planet")
              .attr("transform", "translate(" + orbRadii * Math.sin(interpolateEarthOrbitPosition(t) - earthOrbitPosition.startAngle()()) + "," + -radii.earthOrbit * Math.cos(interpolateEarthOrbitPosition(t) - earthOrbitPosition.startAngle()()) + ")");
